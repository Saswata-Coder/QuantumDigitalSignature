  ...
  return <div className="app-shell">
    <header className="site-header">
      <div className="team-strip container-fluid">
        ...
        <div><div className="team-name">TEAM INNOVATRIX</div><div className="team-sub">Quantum Security Research &amp; Innovation Lab</div></div>
    ...
    <footer className="site-footer"><div><b>Team Members</b><p> · Prince Raj Vishal <br /> · Sidhartha Sarkar <br /> · Harsh Kumar <br /> · Megha Das <br /> · Parna Das <br /> · Saswata Basak</p><small>{/*Add names, roles, emails and institution details here.*/}</small></div><div><b>Resources</b><p><a href="https://qiskit.org/" target="_blank">Qiskit</a> · <a href="https://fastapi.tiangolo.com/" target="_blank">FastAPI</a> · <a href="https://netlify.com/" target="_blank">Netlify</a></p></div></footer>
  </div>
}
...
function Dashboard({ onGo }) {
  return <Section eyebrow="Research demonstrator" title="Controlled teleportation, from entanglement to verification" actions={<Btn onClick={() => onGo('real')}>Launch 3-device lab →</Btn>}>
...
function Statistics() {
  const [family, setFamily] = useState('bell'), [bell, setBell] = useState('Phi+'), [theta, setTheta] = useState(1.1), [phi, setPhi] = useState(.7), [shots, setShots] = useState(2048), [noise, setNoise] = useState(.04), [data, setData] = useState(null); async function run() { const r = await fetch(`${API}/api/teleportation`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ family, bell_state: bell, theta, phi, shots, noise_probability: noise, seed: 17 }) }); setData(await r.json()) } return <Section eyebrow="03 · Statistical verification" title="Bob's state, counts, fidelity and errors" actions={<Btn onClick={run}>Calculate statistics</Btn>}>
